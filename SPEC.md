...
T92|x|write `packages/naics-search-react/README.md`: API docs, usage example, link back to repo|T90
...
T102|x|verify e2e (V27) + visual brand check post-migration|V67,T98,T99,T100,T101
...
B14|2026-08-21|`publish-naics-search.yml`'s real first CI run (tag `naics-search-v2.0.0`) failed @ `pnpm run check`: `apps/naics-resolver`'s typecheck (part of workspace-wide `vp check`) ! resolve `@cajuncodemonkey/naics-search-react`'s types — that pkg's `dist/` was never built in this workflow (only `naics-search`'s build step ran). Same bug class as B5 (fresh CI checkout has ⊥ pre-built `dist/`), now recurring for the 2nd pkg the app depends on. Local runs never caught it (dev machine already had `dist/` built from earlier manual work). Fixed: both publish workflows build `naics-search` then `naics-search-react` (dependency order) BEFORE `pnpm run check`|V66
